/* SourceData step adapted from Fuzzy_matching_in_SAS/data/supporting_code/create_source_and_prospects.sas.
   The only change is the library reference (catyums -> WORK) so it runs standalone;
   the datalines, computed columns (ProspectName, SSN), formats and labels are unchanged. */

data work.SourceData;
...
title "SourceData - first rows";
proc print data=work.SourceData(obs=8) label;
  var ProspectID ProspectName SSN City State Occupation;
run;
title "Prospects by State";
proc freq data=work.SourceData;
  tables State / nocum;
run;
title;
